---
name: review-skill
description: Use when auditing or reviewing a skill or agent prompt file — its text quality (removed rules, broken references, bloat, hardcoded values, cross-file consistency, wording contradictions, steering effectiveness) and its procedural logic (resume/idempotency, step-ordering that destroys needed state, cross-step contradictions, broken invariants, unhandled edge cases, dependency graphs, destructive-op safety) plus whole-repo duplication / single-source-of-truth drift. Audits prompt files — SKILL.md, agent .md, output styles — NOT application code. There is one pass and it always runs both layers; pass --report-only to surface findings without applying fixes. Use when the user asks to review or audit a skill, a prompt, an agent file, or a workflow's logic, or runs /review-skill.
---
...
- **Text pass** — single-file text quality: removed rules, broken references, bloat, hardcoded values, cross-file consistency, wording contradictions, and steering effectiveness. Criteria a–x live in `${CLAUDE_PLUGIN_ROOT}/skills/review-skill/references/text-criteria.md`.
...
2. **Text pass — read each target IN FULL, diff it, and apply criteria a–x**

   Read `${CLAUDE_PLUGIN_ROOT}/skills/review-skill/references/text-criteria.md` now — it holds the rating scale and criteria a–x. **Audit from that file, never from memory**, and apply **every** criterion to every target, stating N/A explicitly where one does not apply.
...
- **Keep the two layers in their own files** — text criteria in `text-criteria.md`, lenses in `logic-lenses.md`. Never restate one inside the other, and never inline either back into this file.
