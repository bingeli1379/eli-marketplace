...
5. **Trace to the root cause — the autonomous loop**
   ...
   **First, a cheap breadth classifier — is this isolated or fleet-wide?** When the infra-shaped signal points at a **shared dependency** (a shared Redis / DB / cache cluster, DNS, a shared gateway — anything many services use), run **ONE cross-service query BEFORE drilling deeper**: the same infra exception (e.g. the `*TimeoutException` / connection error), the same window, but with the **`project` filter dropped** (`size: 0` + `track_total_hits` for the count, plus a small sample with `_source: ["project"]` to see the distinct services hit). This is one round-trip and it forks the whole investigation:
   ...
   **Discipline — the breadth query is the ONLY early fork; everything else stays on the chain.** The isolated-vs-fleet breadth query above is the one query you may fire before drilling deeper — and only when the signal genuinely points at a **shared datastore / network layer** (Redis / DB / DNS / shared gateway), NOT when `current` is an ordinary service whose own logs / metrics you are about to read next anyway. For anything else — hunting other root causes, corroboration, sibling services that are not the shared resource — stay on *this* chain (5a→5e) until it reaches the root cause or is genuinely stuck; eager divergence wastes the user's time and tokens. Two hard rules when you do fire the breadth query: (1) it is the **same-signature** query (same exception + window, `project` dropped), **never a generic all-project error count** — that only drags in unrelated chronic noise you then burn queries ruling out; (2) if `current`'s own data already shows it **healthy + serving normal traffic**, shared-saturation is already falsified — **skip the breadth query**. A fleet number whose exception signature or timing does not match the incident is background noise: note it and move on, do not chase it with baseline queries.
