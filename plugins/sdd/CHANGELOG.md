...
### Added
- Changes that alter a data format or an API / event / database contract are now checked on both sides of every boundary: the workflow flags a backend call that breaks because the frontend now sends a value in a new format, and an API or schema change that breaks the code consuming it — before the change ships.
- Planning and review now guard three common production risks: a schema change must come with a data backfill and rollback plan; an operation that can be retried (a payment, a webhook) is flagged when running it twice would double-charge or duplicate; and a new endpoint or job is flagged when it lacks the logging the rest of the code already has.

### Fixed
- Resuming an interrupted implementation run no longer re-does work that was already committed, and no longer marks a half-finished group as complete.
- Batch-completing changes no longer deletes a change that was never implemented — it is kept and reported instead.
- Batch-applying several changes now skips one whose prerequisite failed, instead of building it on top of missing code.
- The workflow now behaves correctly in a folder of multiple repos and in a directory with no git, instead of assuming a single git repo.

### Changed
- Design and review adapt better to non-web projects: the architect tailors its output to game, desktop, and data-pipeline stacks rather than assuming a web API, and the frontend checklist no longer assumes a specific store library or that every language file must carry every translation key.
