...
6. **Phase 1 — Sequential single-writer implementation** (MANDATORY):
   ...
   **For each group (in sequence):**
   ...
   c. **Squash the group's per-task commits into one clean commit:**
      ...
      4. Update `tasks.md` checkboxes for this group's tasks (Edit `- [ ]` → `- [x]` for this group only). **Multi-repo / no-git**: `tasks.md` lives at the umbrella cwd, not inside any child repo — write the checkbox update to disk, but only *commit* it when the cwd is itself a git repo (per `${CLAUDE_PLUGIN_ROOT}/references/repo-topology.md`); when it is not, the on-disk `tasks.md` plus the per-child-repo code commits ARE the durable state, so skip the commit below. When the cwd IS a git repo, commit with a **clean staging area**:
   ...
   **Fix → Re-verify Loop (max 3 rounds):**
   ...
   1. **Collect all issues** from all reviewers that have completed so far. Group by responsible agent (e.g., backend issues → dotnet-engineer, frontend issues → vue-engineer).
      ...
      **Then read the severities, because they decide whether another review round is owed.** A verdict string alone does not: `ISSUES FOUND` is returned for a `minor` just as it is for a `blocker`.
      - **Any `blocker` / `major`, a FAILED QA run, or a CRITICAL capacity regression** → fix, then a full fresh review round (step 3). Fixing something that severe is exactly where a fix-introduced bug is likely, so the round is earning its cost.
      - **`minor` / `low` only** → fix what is worth fixing, **verify the fix diff yourself (step 2b, the fix-diff check), and stop the loop there** — do NOT spend another full trio on it. Record the untouched items as follow-ups in your report so nothing disappears.
   ...
   4. If the fresh review finds NEW issues → triage them by severity exactly as in step 1: a new `blocker` / `major` repeats the loop (collect → sequential fix → fresh review); new `minor` / `low` ones are fixed, self-verified at step 2b (the fix-diff check), and recorded — they do not buy another round.
   5. The loop ends when no `blocker` / `major`, FAILED QA, or CRITICAL capacity regression remains — or 3 rounds are reached. **Outstanding `minor` / `low` findings do NOT hold it open**: a verdict of `ISSUES FOUND` carrying only minors is a finished loop, not a failing one, so it satisfies this exit.
   6. If a `blocker` / `major` still stands after 3 rounds, pause and report ALL remaining issues to the user. Unfixed minors are never a reason to pause — they go into the report as follow-ups.
