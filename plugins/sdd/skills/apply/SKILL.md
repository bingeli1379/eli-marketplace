...
7. **Dispatch worker agents (following orchestrator.md rules)**
   ...
   **Zero-misses principle: orchestrator ALWAYS dispatches every phase — the agent decides scope, not you.** Do NOT skip any phase based on your own judgement (e.g., "this is just a migration", "changes are mechanical", "only config files changed"). If there is genuinely nothing to do, the dispatched agent will report that. The ONLY way to skip a phase is if `config.yaml` explicitly provides a skip option for it.
   ...
   If review, security, or QA fails: **collect all issues from all reviewers**, group by responsible agent, then dispatch **fix agents sequentially** — one responsible agent at a time, each committing before the next (fixing is a write task and stays single-threaded). **Read each fix's diff yourself and confirm that specific defect is gone before accepting it** — an agent reporting `DONE` over a green test suite can still have closed only part of a multi-part finding, and nothing else in the pipeline sees that. Then **triage by severity**: a `blocker` / `major` / FAILED QA / CRITICAL regression earns a **fresh review round** (all three reviewers again, newly spawned, simultaneously, read-only), **scoped to `git diff <previous round's HEAD>..HEAD` rather than to every changed file** — the fix range plus the importer/dependent scan the code reviewers already run is where a fix-introduced bug lands, and the round's report states the range it covered. Findings that are only `minor` / `low` end the loop once fixed and self-verified, recorded as follow-ups rather than spending another trio; **that branch is terminal — the fixes it dispatches do not buy a review round back**. Max 3 rounds; only pause and report to the user if a blocker/major still stands after them. Full rules: `agents/orchestrator.md` → *Fix → Re-verify Loop*.
...
- If review, security, or QA fails: collect all issues, group by responsible agent, dispatch **fix agents sequentially** (one write agent at a time), **verify each fix's diff closes the finding** — then a fresh review round, scoped to the fix range, only when a `blocker` / `major` / FAILED / CRITICAL was fixed; `minor`-only findings end the loop once fixed, and no reviewer follows those fixes (max 3 rounds). Only pause and report if a blocker/major still stands.
