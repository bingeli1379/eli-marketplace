...
| `## External Boundary Contracts` section | SHOULD exist when proposal.md signals a value representation/format change (keywords in What Changes / Impact: rename, format, enum, serialize/serialization, type change, unit) that crosses a boundary. WARN if missing — an untraced egress is the most common cause of post-merge API breakage |
