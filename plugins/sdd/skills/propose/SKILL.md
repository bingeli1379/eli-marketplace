...
7. **Generate artifacts in dependency order**
   ...
   **A task is work a dispatched agent performs. Everything else goes in `## 交付後由你執行`, at the end of the file, outside the numbered groups.**
   ...
   The split is by **who acts**, not by how technical the work is: whatever completion depends on the user — triggering an environment deployment, approving a release, entering a credential only they hold, clicking through a third-party console, observing a dashboard that exists only in production — is theirs, and no agent can ever tick its box. Written as a numbered task it becomes a checkbox `/apply` can never close: a runtime-upgrade change shipped 18 tasks of which 5 were stg/uat/prod deployments the user triggers by hand, and the run ended at 13/18 with five permanently open boxes, reporting a finished change as incomplete.
