...
6. **Clarify requirements and define feature boundaries**
   ...
   **e. Ask the user** via **AskUserQuestion** — use a **chain-form** message that the user can scan in one screen. Goal: surface only what the user needs to decide on; hide what's already obvious.
   ...
   規模與執行環境：<資料量級 · timeout/gateway 等硬限制(可否調整) · 取代的既有流程>
   ...
   **Format guardrails for the chain message**:
   ...
   - **`規模與執行環境` is MANDATORY whenever the change iterates an unbounded population or performs bulk external mutation** (batch API writes, mass updates, migrations, backfills); omit the line only for genuinely bounded work. Ask three things and do not let them arrive late: **expected volume** (records per run the user will actually submit), **hard runtime limits** (gateway/proxy timeout, request size, rate limit — and explicitly whether each is adjustable), and **which existing process this replaces**. These reshape a design more than any feature question: volume decides whether a synchronous request can complete at all, a fixed timeout decides whether the HTTP response is even a usable output channel, and the legacy process is the de-facto spec. Learning them after `design.md` exists means re-running the architect and re-deciding the execution model — the most expensive mistake this step can make. It does not overlap `NFR/Data/Reversibility` two lines below: that one states **your** conclusion about targets and blast radius, this one asks for **environment facts only the user has**.
