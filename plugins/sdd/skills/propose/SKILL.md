...
6. **Clarify requirements and define feature boundaries**
   ...
   **Format guardrails for the chain message**:
   ...
   - **`規模與執行環境` is MANDATORY whenever the change iterates an unbounded population or performs bulk external mutation** (batch API writes, mass updates, migrations, backfills); omit the line only for genuinely bounded work. Ask three things and do not let them arrive late: **expected volume**, **hard runtime limits** (gateway/proxy timeout, request size, rate limit — and explicitly whether each is adjustable), and **which existing process this replaces**. These reshape a design more than any feature question: volume decides whether a synchronous request can complete at all, a fixed timeout decides whether the HTTP response is even a usable output channel, and the legacy process is the de-facto spec. Learning them after `design.md` exists means re-running the architect and re-deciding the execution model — the most expensive mistake this step can make. It does not overlap `NFR/Data/Reversibility` two lines below: that one states **your** conclusion about targets and blast radius, this one asks for **environment facts only the user has**.
   - **Volume means the number the machine actually holds, and it needs a ceiling.** Two failures, both of which produce a design that looks scale-aware and still OOMs. **(a) Wrong number.** When each unit of input fans out into a queried population — one file row → a query/SP returning N members, one id → its whole history — the submitted count is not the volume; the **derived count per unit of input** is, and it is the one to ask for. Ask for both when they differ. **(b) A floor answered instead of a ceiling.** 「十萬以上」「幾萬吧」 is a lower bound, and a design whose peak memory or runtime is O(that population) cannot be judged against a lower bound — nothing in it rules out ten million. Press for the number the design must survive; if a lookup tool can count the real population (a `COUNT` against the actual query, a project-knowledge skill), use it rather than accept a hand-wave; if it stays unknown after both, it goes in `未定` with the consequence spelled out, and passes to the architect as an estimate labelled as one — never as a settled number.
   - **Ask with the consequence attached, and never hand over a bare menu of numbers.** A naked 「大概幾筆？」 asks the user to supply an input whose purpose only you can see, so they answer from feel — and when the options came from you, picking the largest is a shrug, not a confirmation. Say in the same breath **what breaks at what size and what you would build differently**: 「同步請求 + 一次撈進記憶體，十萬還撐得住，百萬會 OOM／逾時；超過那條線我就改成分頁串流或非同步 job」. That turns a number they do not have into a decision they do — most users cannot tell you the row count, but every one of them can tell you 「不可能到百萬」. Where you offer bands, each band carries its consequence, never a bare list of figures. The consequence rides on the question itself — it does not buy the chain message extra lines.
   - **A scale answer that changes nothing is a question you should not have asked.** Once the number is in, it must show up as something: the execution model it selected, the mitigation it forced, or a risk recorded as knowingly accepted with the size that triggers it. Recording it in `design.md` and building exactly what you were already going to build is the failure this whole item exists to prevent — and it is invisible afterwards, because the spec looks thoroughly researched either way.
...
7. **Generate artifacts in dependency order**
   ...
   **Grouping principle: each group = one reviewable unit = one final commit.**
   ...
   - **Grouping follows the implementation strategy design.md chose (Step 7c) — read it, do not re-decide it here.**
     - **Contract-First (default)** → the vertical layer grouping described below (contract-defining layer first, then its consumers).
     - **Walking Skeleton** → group **1** is the end-to-end skeleton (`## 1. 端到端骨架（hardcode 打通）`), covering the exact path design.md named, with placeholders in every layer; groups **2..N** are the per-layer harden groups, each annotated `<!-- depends: 1 -->` (plus any harden-to-harden dependency), in the harden order design.md gave. Rules that must be reflected in the tasks:
       - The skeleton group's tasks state that **every placeholder carries a `SKELETON:` marker comment** (e.g. `// SKELETON: replace in harden phase`).
       - The skeleton group's tasks do **not** include unit-test (RED/GREEN) steps — it is an integration probe (`test-driven-development` → *Walking Skeleton*). It MUST still contain a task that proves the end-to-end path actually runs.
       - **Every placeholder site named in the skeleton group MUST be covered by a harden task** that replaces it, and harden tasks DO follow the RED → GREEN → REFACTOR structure below. A `SKELETON:` site with no harden task is an error `/validate` rejects.
       - Group 1 still obeys every pipeline invariant — single agent type, single repo, one reviewable commit.
   ...
   - **Also check the other direction — a group is not free.** Every group costs `/apply` one full agent dispatch, and writes are serialized, so each one re-reads `design.md` and its specs before writing a line. The rule above only ever pushes toward splitting; left alone it produces a long chain of three-task groups whose setup cost exceeds their work. So after grouping, look for **adjacent groups that share one agent type, have no dependency between them, and are individually small** — and merge them when the result is still describable as one reviewable concern (one honest commit message covers it). Keep them separate when they are genuinely separate concerns; the extra dispatch is the price of a clean commit, and that is a price worth paying. What is not worth paying is a separate dispatch for what a single commit would have described anyway.
