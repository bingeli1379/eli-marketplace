...
| `/review [target] [lens]` | Read-only standalone review by lens (quality/security/performance/e2e/all); auto-detects lens; no edits, no commits. **No target → architecture health scan**: git-log hot spots + deletion test → ranked candidate list handed to `/propose` |
