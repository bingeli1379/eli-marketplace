...
| `tasks.md` checkbox commit (`chore: mark group N`) | commit in the cwd repo alongside the code | write the checkbox update to `tasks.md` on disk; commit it **only if the umbrella cwd is itself a git repo** — otherwise leave it uncommitted on disk. The on-disk `tasks.md` + each child repo's code commits together ARE the durable resume state; do not attempt to commit `tasks.md` into a child repo (it lives at the umbrella, not inside any repo). |
