...
- **Flow and knowledge separate** — `SKILL.md` carries the procedure (order, branching, handoffs, done-conditions) and names the body of knowledge each step applies; that unit states its rules and never names the step that called it, which is what lets it be swapped without rereading the flow. Extraction still needs a reason — an enumeration the world keeps adding to counts, so does a second owner or a real variant; an imagined one does not
