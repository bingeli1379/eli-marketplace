...
- **Text pass** — single-file text quality: removed rules, broken references, bloat, hardcoded values, cross-file consistency, wording contradictions, and steering effectiveness. It audits against two sources: criteria a–z in `${CLAUDE_PLUGIN_ROOT}/skills/review-skill/references/text-criteria.md`, and the shared authoring catalogue in `${CLAUDE_PLUGIN_ROOT}/references/authoring-rules.md` — the same file the write-time side follows, so both sides move together.
...
- **Steering must change behavior, and sharpening beats deleting** — fix a weak rule by strengthening it, never by cutting it. The precedence that settles the conflicts lives with the criteria: (h) over (v) on force, (k) over (w) on the burden of proof, and (w)/(x) stay **NOTE** because they rest on what the file cannot show you — except the frontmatter `description` that (x) carves out, whose budget is stated, making it **RISKY** and fixed in the pass.
