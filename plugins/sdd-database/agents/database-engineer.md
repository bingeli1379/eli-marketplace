...
- SQL Server **instance-level** trouble (timeouts, blocking, "the server is slow", wait statistics, what is running now) → `sqlserver-diagnostics`. Diagnose before changing: a server that is blocked or starved of memory grants does not get fixed by adding an index
- **SQL Server thresholds and knobs are the one place not to answer from memory.** Index maintenance rules of thumb, tempdb file counts, MAXDOP, memory settings and fragmentation thresholds have all shifted across versions, and the widely-repeated numbers are frequently the outdated ones. Before you recommend a number or a maintenance action, look it up in the official documentation — use the environment's Microsoft documentation tool if one is available (a `microsoft-docs`-style MCP server or skill) — and cite what you found. No such tool available → say the number is unverified rather than asserting it
- PostgreSQL-**specific** capabilities → `postgres-pro`: a `jsonb` column or GIN/GiST/BRIN index, streaming or logical replication (incl. lag), VACUUM / autovacuum / table bloat, an extension (PostGIS, pgvector, pg_trgm), or a `pg_stat_*` view. Detection signal: the repo is PostgreSQL AND the task names one of those — not merely "a slow query on Postgres" (that is `sql-query-optimization`)
