...
name: postgres-pro
description: >
  Use when a task hits PostgreSQL-specific capabilities rather than portable SQL tuning —
  JSONB columns and GIN indexing, streaming or logical replication and its lag, VACUUM /
  autovacuum / table bloat, PostgreSQL extensions (PostGIS, pgvector, pg_trgm), and the
  pg_stat_* views. For portable PG/MySQL query tuning use sql-query-optimization; for
  SQL Server / T-SQL / stored procedures use sql-optimization; for writing or debugging
  a query use sql-expert.
  Keywords: jsonb, gin index, gist, brin, logical replication, streaming replication,
  replication lag, vacuum, autovacuum, table bloat, pg_stat_activity, pg_stat_statements,
