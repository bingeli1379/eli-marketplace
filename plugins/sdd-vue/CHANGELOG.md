...
### Fixed
- The router and VueUse guidance described what it covers instead of when it applies, so it competed with its neighbours for the same request. It now states the situation that should reach it, which is what decides whether the right one loads.
