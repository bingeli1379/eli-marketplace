...
**Renaming a reference file or renumbering steps: sweep the mentions that do NOT look like paths.** A rename updates `references/<name>.md` occurrences easily and leaves bare `` `<name>.md` `` cross-references and glued `stepN` citations behind, pointing at nothing. Check #7 above now fails on both, so run it after any rename — that is the cheap half of the problem. The expensive half is what a *self*-review keeps missing: see `/review-workflow` Lens A class 9 (an insertion invalidating a declared budget, an artifact contract, an ordering gate, or the sentence next to it).
